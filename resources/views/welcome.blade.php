<!DOCTYPE html>
<html lang="uz">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="description" content="Ramazon oyida kunlik ibodatlaringizni belgilab boring">
    <title>Ramazon Tracker — Kunlik Ibodat Ilovasi</title>
    <link rel="stylesheet" href="{{ asset('css/app.css') }}">
</head>
<body class="theme-guest">
    <div class="welcome-wrapper">
        <span class="welcome-moon">🌙</span>
        <h1 class="welcome-title">Ramazon Tracker</h1>
        <div class="welcome-arabic">رمضان مبارك</div>
        <p class="welcome-desc">
            Ramazon oyida kunlik ibodatlaringizni belgilab boring.
            Namoz, ro'za, Qur'on, zikr va sadaqalaringizni hisobga oling,
            maqsad qo'ying va o'z taraqqiyotingizni kuzating.
        </p>

        <div class="welcome-actions">
            <a href="{{ route('register') }}" class="btn btn-gold">✨ Ro'yxatdan o'tish</a>
            <a href="{{ route('login') }}" class="btn btn-outline">Kirish</a>
        </div>

        <div class="welcome-features">
            <div class="card welcome-feature">
                <span class="feature-icon">🕌</span>
                <h3>5 Mahal Namoz</h3>
                <p>Har kuni namazlaringizni belgilang</p>
            </div>
            <div class="card welcome-feature">
                <span class="feature-icon">📖</span>
                <h3>Qur'on O'qish</h3>
                <p>Necha sahifa o'qiganingizni yozing</p>
            </div>
            <div class="card welcome-feature">
                <span class="feature-icon">🎯</span>
                <h3>Maqsadlar</h3>
                <p>Ramazon uchun maqsad qo'ying</p>
            </div>
            <div class="card welcome-feature">
                <span class="feature-icon">📊</span>
                <h3>Hisobotlar</h3>
                <p>Haftalik va oylik tahlil</p>
            </div>
        </div>
    </div>
</body>
</html>
