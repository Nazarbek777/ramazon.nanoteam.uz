        ...
        <aside class="relative w-72 bg-white border-r border-slate-200 transition-all duration-300 flex flex-col"
            ...
            <div class="h-20 flex items-center px-6 border-b border-slate-100">
                ...
                <span class="ml-3 font-bold text-xl tracking-tight transition-opacity duration-300"
                    :class="sidebarOpen ? 'opacity-100' : 'opacity-0 lg:hidden'">ADMIN<span
                        class="text-indigo-600">PRO</span></span>
            </div>

            <!-- Navigation -->
            <nav class="flex-1 overflow-y-auto p-4 space-y-1">
                <p class="text-[10px] uppercase font-bold text-slate-400 px-3 mb-2" :class="!sidebarOpen && 'hidden'">
                    Asosiy</p>
                ...
                @if(auth()->user()->hasPermission('broadcast.view'))
                    ...
                    <a href="{{ route('admin.live-stream.index') }}"
                        class="sidebar-link group flex items-center px-4 py-3 rounded-xl transition duration-200 hover:bg-slate-50 {{ request()->routeIs('admin.live-stream.*') ? 'active' : '' }}">
                        <i class="fas fa-video w-6 text-indigo-500"></i>
                        <span class="ml-3 font-semibold" :class="!sidebarOpen && 'lg:hidden'">Live Stream</span>
                    </a>
                @endif
                ...
                @if(auth()->user()->isSuperAdmin())
                    <div class="pt-2">
                        <p class="text-[10px] uppercase font-bold text-slate-400 px-3 mb-1"
                            :class="!sidebarOpen && 'hidden'">Super Admin</p>
            ...
            <div class="p-4 border-t border-slate-100">
                <form action="{{ route('admin.logout') }}" method="POST">
                    @csrf
                    <button type="submit"
                        class="w-full flex items-center px-4 py-3 rounded-xl text-red-500 hover:bg-red-50 transition duration-200 font-bold">
