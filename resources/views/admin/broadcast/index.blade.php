    ...
    <div class="max-w-4xl mx-auto">
        <div class="flex items-center justify-between mb-8">
            ...
            <div class="bg-indigo-50 px-6 py-3 rounded-2xl border border-indigo-100 flex items-center">
                ...
                <div>
                    <p class="text-[10px] uppercase font-black text-indigo-400 tracking-widest leading-none mb-1">Jami
                        a'zolar</p>
        ...
        <div class="bg-white rounded-[32px] shadow-xl shadow-indigo-100/20 border border-white overflow-hidden">
            <div class="p-8">
                <form action="{{ route('admin.broadcast.send') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <!-- Mode Switcher -->
                    <div class="flex p-1 bg-gray-100 rounded-2xl mb-8 max-w-sm">
                        <button type="button" @click="mode = 'classic'"
                            :class="mode === 'classic' ? 'bg-white shadow-sm text-indigo-600' : 'text-gray-500'"
                            class="flex-1 py-3 px-4 rounded-xl text-sm font-bold transition-all">
                            Classic
                        </button>
                        <button type="button" @click="mode = 'link'"
                            :class="mode === 'link' ? 'bg-white shadow-sm text-indigo-600' : 'text-gray-500'"
                            class="flex-1 py-3 px-4 rounded-xl text-sm font-bold transition-all">
                            Link orqali (Copy)
                        </button>
                    </div>

                    <div x-show="mode === 'classic'" x-transition>
                        <!-- Message Body -->
                        <div class="mb-8">
                            <label for="editor" class="block text-sm font-bold text-gray-700 mb-2 ml-1">Xabar matni</label>
                            <div
                                class="rounded-2xl overflow-hidden border border-gray-100 shadow-sm focus-within:ring-4 focus-within:ring-indigo-500/10 transition-all">
                        ...
                        <div class="mb-8">
                            ...
                            <div class="flex items-center justify-center w-full">
                                <label for="image"
                                    class="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-200 border-dashed rounded-[20px] cursor-pointer bg-gray-50 hover:bg-gray-100 transition-all group">
                                    <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                        <i
                                            class="fas fa-cloud-upload-alt text-2xl text-gray-400 group-hover:text-indigo-500 mb-2 transition-colors"></i>
                                        <p class="mb-2 text-sm text-gray-500"><span
                                                class="font-bold text-indigo-600">Yuklash uchun bosing</span> yoki rasmni
                                            sudrab keling</p>
                                        <p class="text-[10px] text-gray-400 font-bold uppercase tracking-wider">PNG, JPG,
                                            JPEG (Maks. 5MB)</p>
                                    </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                            <div>
                                <label for="button_text" class="block text-sm font-bold text-gray-700 mb-2 ml-1">Tugma
                                    matni</label>
                            ...
                            <div>
                                <label for="button_url" class="block text-sm font-bold text-gray-700 mb-2 ml-1">Tugma linki
                                    (URL)</label>
                    ...
                    <div x-show="mode === 'link'" x-transition style="display: none;">
                        <div class="mb-10">
                            <label for="message_link" class="block text-sm font-bold text-gray-700 mb-2 ml-1">Telegram xabar
                                linki</label>
                            <input type="text" name="message_link" id="message_link"
                                class="w-full px-5 py-4 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 outline-none transition-all text-gray-700 placeholder:text-gray-300"
                                placeholder="Masalan: https://t.me/kanal_nomi/123">
                            <p
                                class="text-[10px] text-gray-400 mt-2 ml-1 uppercase font-bold tracking-widest leading-relaxed">
                                <i class="fas fa-info-circle mr-1"></i> BOT SHU KANALDA ADMIN BO'LISHI YOKI KANAL OCHIQ
                                BO'LISHI KERAK
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-end">
                        <button type="submit"
                            onclick="return confirm('Haqiqatan ham barcha foydalanuvchilarga xabar yubormoqchimisiz?')"
            ...
            <div class="bg-indigo-50/30 p-8 border-t border-indigo-50 flex items-start space-x-6">
                <div
                    class="w-14 h-14 bg-white rounded-2xl flex items-center justify-center text-indigo-600 shrink-0 shadow-sm">
                ...
                <div class="flex-1">
                    ...
                    <p class="text-sm text-indigo-600/70 leading-relaxed">
                        Editor orqali matnni chiroyli ko'rinishga keltiring. Agar rasm yuklasangiz, matn rasmning tagida
                        <b>caption</b> sifatida ko'rinadi.
