...
<div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 mb-8">
    ...
    <a href="{{ route('admin.quizzes.index', ['subject_id' => $isSelected ? '' : $subject->id]) }}"
...
<div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
    <div class="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-indigo-50/50">
        <div class="flex items-center gap-2">
            ...
            @endif
            <h4 class="font-bold text-gray-800">{{ $selectedSubject->name }} — testlar</h4>
        </div>
        <a href="{{ route('admin.quizzes.create', ['subject_id' => $selectedSubject->id]) }}"
    ...
    @endphp
    <div class="flex items-center justify-between px-5 py-3.5 border-b border-gray-50 last:border-0 hover:bg-gray-50/50 {{ $isExpired ? 'opacity-60' : '' }}">
        <div class="flex-1 min-w-0">
            <p class="font-semibold text-gray-800 text-sm">{{ $quiz->title }}</p>
            <p class="text-xs text-gray-400 mt-0.5">
                {{ $quiz->time_limit }} daq &bull; O'tish: {{ $quiz->pass_score }}%
                @if($quiz->access_code)
                    &bull; <span class="font-mono bg-gray-100 px-1 rounded text-[10px]">{{ $quiz->access_code }}</span>
                @endif
            </p>
        </div>
        <div class="flex items-center gap-3 ml-4 shrink-0">
            ...
            @endif
            <a href="{{ route('admin.quizzes.edit', $quiz) }}" class="text-indigo-400 hover:text-indigo-700 text-sm">
    ...
    @empty
    <div class="px-5 py-8 text-center text-gray-400 text-sm italic">Bu fanda hali test yo'q.</div>
    @endforelse
</div>
@else
<div class="bg-white rounded-2xl border border-dashed border-gray-200 py-10 text-center text-gray-400 text-sm">
    ...
    Fan kartochkasini bosing — uning testlarini ko'ring
</div>
@endif
@endsection
